...
void MPU9250::readAccelData(int16_t * destination, uint8_t address)
{
...
void MPU9250::readGyroData(int16_t * destination, uint8_t address)
{
...
int16_t MPU9250::readTempData(uint8_t address)
{
...
void MPU9250::initMPU9250(uint8_t address)
{
 // wake up device
...
void MPU9250::calibrateMPU9250(float * gyroBias, float * accelBias, uint8_t address)
{
...
void MPU9250::MPU9250SelfTest(float * destination, uint8_t address) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
  ...
  writeByte(address, GYRO_CONFIG, 1<<FS);  // Set full scale range for the gyro to 250 dps
  writeByte(address, ACCEL_CONFIG2, 0x02); // Set accelerometer rate to 1 kHz and bandwidth to 92 Hz
  writeByte(address, ACCEL_CONFIG, 1<<FS); // Set full scale range for the accelerometer to 2 g
